...
public class StatisticsCollector implements IFloodlightModule, IStatisticsService {
				...
				for (OFStatsReply r : e.getValue()) {
					...
					for (OFPortStatsEntry pse : psr.getEntries()) {
						...
						if (portStats.containsKey(npt) || tentativePortStats.containsKey(npt)) {
							...
							Long bwRx = spb.getBitsPerSecondRx().getValue();
							Long bwTx = spb.getBitsPerSecondTx().getValue();

							if(bwRx >PortRxThreshold){
								log.info("Rx: Se superó el umbral {}>{}", bwRx,PortRxThreshold);
							}

							if(bwTx >PortTxThreshold){
								log.info("Tx: Se superó el umbral {}>{}", bwTx,PortTxThreshold);
							}
			...
			for (Entry<DatapathId, List<OFStatsReply>> e : replies.entrySet()) {
				for (OFStatsReply r : e.getValue()) {
					...
					for (OFFlowStatsEntry fse : fsr.getEntries()) {
						...
						log.info("Contador de Bytes: {}", fse.getByteCount());
						log.info("Idle Timeout: {}", fse.getIdleTimeout());
						log.info("Duración: {} ", fse.getDurationSec());
					}
				}
			}
		}
	...
	public void init(FloodlightModuleContext context)
		...
		PortTxThreshold = Integer.valueOf(config.get("PortTxThreshold"));
		PortRxThreshold = Integer.valueOf(config.get("PortRxThreshold"));

		log.info("PortTxThreshold : {}", PortTxThreshold );
		log.info("PortRxThreshold : {}", PortRxThreshold );
